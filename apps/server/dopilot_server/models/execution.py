...
⚠️ **Wire/disk/agent seam (do NOT rename here).** The Redis stream payloads, the
on-disk log path, and the agent state files still use ``execution_id`` (= the
TASK id) and ``attempt_id`` (= the atomic EXECUTION id). So the seam tables
:class:`ExecutionLogFile` (and ``command_outbox`` / ``event_audit``) keep the
columns ``execution_id`` / ``attempt_id`` — boundary code in the services maps
``Task.id -> execution_id`` and ``Execution.id -> attempt_id``. See the seam
note in :mod:`dopilot_protocol.streams`.
...
class Execution(Base):
    ...
    This is the agent's idempotency unit; on the Redis/disk/agent seam it is
    still called ``attempt_id``. ``task_id`` is the parent :class:`Task`.
    """
...
class ExecutionLogFile(Base):
    ...
    These are SEAM columns (wire/disk identifiers), kept verbatim in phase 1.7:
    ``execution_id`` holds the parent :class:`Task` id and ``attempt_id`` holds
    the atomic :class:`Execution` id. Do NOT rename them — they match the agent
    wire and the on-disk ``{execution_id}/{attempt_id}.log`` path.
    """
