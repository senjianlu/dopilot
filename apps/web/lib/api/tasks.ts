...
// as a query param when web auth is on. Honors NEXT_PUBLIC_API_BASE so a dev
// pointing at a remote server still streams from the right origin.
export function buildStreamUrl(id: string, query: StreamUrlQuery = {}): string {
  ...
  return `${base}/tasks/${id}/logs/stream?${params.toString()}`;
}
