...
>
> 本文面向参与 dopilot 改造的工程师，详尽剖析 scrapydweb 的「定时任务（Timer Tasks）」子系统：它基于 APScheduler 的 `BackgroundScheduler` 在 Flask 进程内调度作业，是 dopilot scheduler 的**行为参考核心**。
>
> 文中区分两类内容：
...
这是整个子系统最微妙的部分，dopilot 自有 scheduler 实现时需复刻这套行为语义。
...
**线程上下文【gotcha】**：`execute_task` 与 `TaskExecutor` 的**每个** DB 操作都单独包在 `with db.app.app_context()` 中（`execute_task.py:65,107,126,151`）。原因是 SQLite 限制「对象只能在创建它的线程使用」，代码用「每次新建 app_context」规避。dopilot 实现时需复刻此行为约束。
...
>
> **【已被通信重构取代 · superseded-by】** 本节关于 **推模式（#4）下发**、**实时日志流（#5）**、**节点策略（#2）健康来源** 的 dopilot 改造口径，以 [`../refactor/00-redis-streams-agent-communication.md`](../refactor/00-redis-streams-agent-communication.md) 为准。要点：server→agent 不再走「进程内 test_client / 直连 agent HTTP」主路径，而是 server `XADD` 命令到 Redis Streams（`dopilot:agent:{agent_id}:commands`），agent 主动 consumer group 消费；实时日志由 agent 主动 `XADD` 到 Redis log stream、server 消费后落盘 + SSE 推 Web，不再由 server 主动 pull；节点健康来源由轮询改为 agent heartbeat 写 `nodes.last_seen_at`。下表 #4/#5/#2 单元格已据此更新；其余行（scrapydweb 现状落点）不受影响。
...
| 2 | 节点策略「随机选一个执行」 | `execute_task.py:44,168`（遍历全部 selected_nodes） | `Task` 增 `node_strategy`（all\|random）列；`random` 时读 Task 后裁剪为单节点。**三态语义不变，但在其上叠加 heartbeat 健康过滤**：候选节点先按 `healthy = now - nodes.last_seen_at <= heartbeat_timeout_seconds` 过滤（健康来源由轮询 `/health` 翻转为 agent heartbeat 写 `last_seen_at`），再按 all/random 选取。此接缝由 `node_strategy` 抽象承载，详见 [`../refactor/00-redis-streams-agent-communication.md`](../refactor/00-redis-streams-agent-communication.md) |
...
| 4 | 推模式（push）主动下发 | `execute_task.py:82,88`（`/1/→/N/` + test_client）、`tasks.py:353-364`（fire） | 「指定节点 + 立即触发」即雏形。dopilot 做不依赖定时器的「立即推送」入口，但**不再把进程内 test_client 调用替换为 server→agent 的直连 HTTP 下发**：server 在同一 PG 事务内写 execution/attempt/`command_outbox`，再由 dispatcher `XADD` `run` 命令到目标 agent 的 Redis 命令 stream（`dopilot:agent:{agent_id}:commands`），agent 主动消费并启动。此即 `BaseExecutor.run_on_node` 的最终形态（由 POST /run 改为 XADD command）。详见 [`../refactor/00-redis-streams-agent-communication.md`](../refactor/00-redis-streams-agent-communication.md) |
| 5 | 实时日志流 | `schedule.py:48-50` / `tasks.py:22-24`（仅 `send_file` 静态日志）、`scheduler.py:19`（写文件）、`views/files/log.py`（解析 scrapyd 日志） | 现状是文件下载式，非实时。dopilot v1 实时日志走 Redis log stream（详见 [`03-gap-realtime-logs.md`](../dopilot/03-gap-realtime-logs.md) 与 [`../refactor/00-redis-streams-agent-communication.md`](../refactor/00-redis-streams-agent-communication.md)）：agent 主动 `XADD` 日志增量（base64 字节 + offset/eof）到 `dopilot:server:logs`，server 消费后正文落 `/server-data/logs`、索引/offset/状态落 PG，再经 **server→web SSE** 推 Vue（不引入 WebSocket）。日志回流方向由「server pull agent tail」翻转为「agent push」；`LogSource` 抽象保留，实现由 `AgentTailLogSource` 换为 `RedisLogSource` |
| 6 | i18n（当前界面/flash 英文硬编码） | `schedule.html` / `tasks.html` 模板、各视图 `flash()` / `js['tip']` / `msg` 英文串（如 `tasks.py:80,319,363`） | dopilot i18n 走**前端 vue-i18n**(`apps/web`,见 `../dopilot/04-gap-i18n.md`),**不走后端 gettext**;此处仅为「scrapydweb 后端文案分布」的行为参考 |
| 7 | jobstore 持久化后端切换 | `scheduler.py:31-34`、`setup_database.py`（计算 URI） | 改 `DATABASE_URL` 即可让 jobstore 走 MySQL/PG；executor 可换 `ProcessPoolExecutor`（`scheduler.py:7,37` 已留注释）。**dopilot 决策 #10：jobstore 仍走 PostgreSQL，不为 jobstore 引入 Redis。** ⚠️ 这里的「APScheduler jobstore 用的 Redis」与通信重构引入的「server↔agent 通信总线 Redis」（[`../refactor/00-redis-streams-agent-communication.md`](../refactor/00-redis-streams-agent-communication.md)）是**两个互不相干的用途**：后者是消息总线/瞬时传输（命令/事件/日志 stream），不是 dopilot 持久化数据库，也不承载 APScheduler 作业状态——不要把通信 Redis 误当作 jobstore 后端 |
...
> 【已由 v1 单实例硬约束（单容器 + uvicorn workers=1 + 单 APScheduler）关闭，不做多副本 / 不引入分布式锁】下列方案保留为 scrapydweb 现状与历史决策记录。下方第 2 项「基于 Redis/DB 选主」属被关闭的多副本路线，**不予采纳**。
>
> **【口径收窄 · 见通信重构】** 「不引入 Redis」这一约束已收窄为「**不引入 Redis 做多副本 HA / fan-out / 分布式锁选主**」；通信重构（[`../refactor/00-redis-streams-agent-communication.md`](../refactor/00-redis-streams-agent-communication.md)）已**显式允许 Redis 作单实例 server↔agent 通信总线**（命令/事件/日志 stream）。两者不冲突：引入通信 Redis 不表示支持多 server active-active，server→web 的 SSE fan-out 仍在单进程内存完成。
...
## 14. dopilot 复刻 scheduler 行为时需对齐的不变量（行为参考 checklist）
