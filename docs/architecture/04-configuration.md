...
| server | `[agents]` | `heartbeat_timeout_seconds`/`stalled_attempt_seconds`/`lost_after_stalled_seconds`（默认 3600;有运行期心跳后仅约束「agent 在线但存活不可确认」,不是任务时长上限）/`no_progress_stall_seconds`（日志零增长多久后告警,默认 1800,0=关闭;**只告警不判 lost**）/`no_progress_sample_max_age_seconds`（采样有效期,默认 300;读数过期就停止判定——看不见不等于卡住）/`auto_stop_on_no_progress`（默认 **false**,开启后无进度会真的取消任务）/`agent_token` |
