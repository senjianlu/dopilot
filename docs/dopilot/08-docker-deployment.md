...
# dopilot server 依赖声明在 apps/server/pyproject.toml
...
# dopilot server 包：apps/server/dopilot_server/（全新实现，非 scrapydweb 改名）
COPY apps/server/ ./apps/server/
RUN pip install --no-cache-dir --no-deps -e ./apps/server
...
# dopilot agent 包：apps/agent/dopilot_agent/（含 runners/、main.py，全新实现）
COPY apps/agent/ ./apps/agent/
RUN pip install --no-cache-dir --no-deps -e ./apps/agent
...
完整 compose 示例用于集成验收、镜像构建验证和部署演练，不是日常开发的最低要求。
...
| `server` | `5000:5000` | Web/API/SSE 入口。 |
...
agent（阶段 1 即落地）使用 `configs/agent.toml`：
