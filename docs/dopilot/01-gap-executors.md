...
|---|--------|------|------|
| Q1 | **Docker 常驻爬虫的执行通道** | **已锁定 = 方案 B（dopilot-agent + Redis Streams）**：server 不直连 docker daemon，docker SDK 调用全归 agent。方案 C（server 直连 daemon over TLS）已否决 | `DockerExecutor` 经 Redis `run` 命令 → agent；鉴权 = Redis AUTH/ACL + `agent_token` |
