...
| 8 | 代码仓库结构 | **monorepo，apps/packages 布局**：server = `apps/server/dopilot_server`，agent = `apps/agent/dopilot_agent`，web = `apps/web`；server↔agent 共享协议在 `packages/protocol`（可选客户端 SDK 在 `packages/client`），部署物在 `deploy/`，配置样例在 `configs/`。三者**同仓开发，不拆分多仓**。上游 scrapydweb 仅作只读功能/测试参照、外部查阅，本仓库不保留本地快照、不参与构建。权威布局详见 `docs/dopilot/05-dev-setup-and-known-issues.md` §1。 |
| 9 | scrapydweb 参考边界（原则锁定） | scrapydweb 仅作 **(1) 功能层/行为参考** 与 **(2) 测试 oracle**；其**代码写法、目录结构、模块划分、命名、依赖、配置形态一律不得作为 dopilot 的设计依据**。dopilot 为 **greenfield**、按 `apps/`+`packages/` 自有领域 **structure-first** 全新编写：上游 scrapydweb **只读、外部查阅，不被拉取/内置/import、不进构建上下文、不做改名/git mv**（本仓库已移除本地快照）。凡文档中出现的 scrapydweb `file:line` 引用，**仅为行为参考**（相对上游 scrapydweb 1.6.0 / commit `1341cf9`，约定见各文档「Docs convention」），绝非 dopilot 的改动目标。 |
...
| 12 | 认证 / 通信边界 | **Web 管理员认证与 Agent 机器认证分离**。**Web 管理员认证 fail-closed（阶段 2.2 翻案，不再是 config-present-or-off）**：生产启动经 `load_settings()` 时，若未显式禁用且 `admin_username`/`admin_password`/`token_secret` 任一缺失/为空，则抛 `ConfigError` 拒绝启动（不再静默以匿名管理员开放 API）；唯一的开发逃生舱是显式 `DOPILOT_AUTH_DISABLED=true`（即 `AuthSettings.disabled`），此时匿名管理员模式在配置中可见、不再隐式进入。`token_secret` 是登录 access token 与 SSE stream token 的内部 HMAC 签名密钥，**仅 TOML 配置、无 env 覆盖**。**Web→Server**：单管理员登录 + Bearer **opaque access token**（无 refresh token）；另有静态 `admin_api_token`（env `DOPILOT_ADMIN_API_TOKEN`，非空须 >=16 字符）可直接作为 `Authorization: Bearer <token>` 调 admin API，供自动化/CI 使用；SSE 用短期 `stream_token`（TTL 60s、仅校验建连），避免长期 token 放 URL。**Agent 机器认证（阶段 2.2.3 收敛为单令牌）为 config-present-or-off**：唯一机器令牌 `DOPILOT_AGENT_TOKEN`（server `[agents].agent_token` / agent `[agent].agent_token`，非空才校验、非空须 >=16 字符）认证 agent→server 单向：agent 在其出站调用（heartbeat + artifact/wheel 拉取）携带令牌、server 校验。**阶段 2.2.7 起 agent 为纯出站，不存在 server→agent HTTP 方向（原"server→agent 部署 egg"路径已删除，egg 改由 agent 拉取）。** **阶段 2.2.4 在 server 运行时边界放宽**：未配置 `agent_token` 时，server 运行时/CLI 自动生成强令牌（`secrets.token_urlsafe(32)`）并持久化到 `<server.data_dir>/secrets/agent-token`（新增 `[server].data_dir`，默认 `/server-data`，env `DOPILOT_SERVER_DATA_DIR`），重启复用——机器认证经生成令牌即 ON。生成**仅 server 端**（agent 永不生成），是运行时步骤（`dopilot_server.agent_token`），`load_settings()` 保持无副作用（不建文件、不生成）；`create_app(settings)` 把传入 settings 注入 `Depends(get_settings)`，使入站 heartbeat 鉴权读到同一（写入生成令牌的）settings——与 agent 出站调用所持令牌一致。运维用 `dopilot-server agent-token print [--quiet]`（无需 DB/Redis/ASGI，可 `docker exec`）取令牌。部署提供三份 compose：一体（`docker-compose.yml`，显式共享令牌，因服务同时启动）、server-only（`docker-compose.server.yml`，令牌可省→生成）、agent-only 接入（`docker-compose.agent.yml`，令牌必填、绝不下发 admin token）。旧的拆分令牌（`agent_auth.shared_token` / `agents.server_shared_token` 及 `DOPILOT_AGENT_SHARED_TOKEN` / `DOPILOT_SERVER_SHARED_TOKEN`）与"从 `admin_api_token` 回退"已删除、无任何效果；`admin_api_token` **仅管理员、仅 server 端**，绝不下发给 agent、也不充当机器令牌。Token 认证不是传输加密，跨主机加密仍需 TLS/VPN/私有网络。**Server↔Agent（重构后，破坏性、无双轨；详见 `docs/refactor/00-redis-streams-agent-communication.md`）**：通信主路径改为 **Redis Streams**——server→agent 命令（run/stop/cleanup_logs）经 `dopilot:agent:{agent_id}:commands` 投递，**agent 经 consumer group 主动消费命令、主动 XADD 推送状态事件（`dopilot:server:agent-events`）与日志（`dopilot:server:logs`）**，删除 server→agent HTTP run/status/tail 主路径。健康检查改为 **agent 主动 POST `/api/v1/agents/{agent_id}/heartbeat`**（不再由 server 轮询 agent `/health`）。机器鉴权（阶段 2.2.3）用**单一** `agent_token` 认证 agent→server 单向（不再拆分、不从 admin token 回退），Redis 启用 AUTH/ACL。**阶段 2.2.7：agent 成为纯出站守护进程——删除其全部入站 HTTP（含 egg 部署端点与 `/health` 端点）、不再监听任何端口（`6800` 移除）；egg 改由 agent 执行 Redis `run` 命令时从 server 拉取后部署到本机 scrapyd，容器健康检查改用本地 exec `dopilot-agent-healthcheck`。** agent 仍**不直连 PostgreSQL**。agent 启动携带容器重启不变的稳定 `agent_id`，server 将节点落入 `nodes` 表，健康来源由 heartbeat 写入的 `nodes.last_seen_at` 判定。第一版不做 mTLS / token 轮换 / RBAC / 多用户。 |
...
| 15 | 阶段 1.8 领域模型清理 | 使用 **BuildArtifact / ExecutionTemplate / Schedule / Task / Execution** 作为产品与 API 口径；构建产物成为真实 DB 实体；执行模板必须绑定构建产物；public API/Web 硬切到 Task/Execution；`task_type` 仅作为现有 agent wire 字段保留在边界。**阶段 2.2 补注**：`execution_templates.name` 与 `schedules.name` 唯一（创建/改名冲突返回 409）；`schedules` 新增行级 `enabled`（默认 `false`），控制该定时是否被 APScheduler 注册/触发——`enabled=false` 的 schedule 仍可列出/查看/更新/删除并可经 `trigger-now` 手动运行（即"暂停/恢复定时触发"已落地，不再 out of scope）。注意行级 `schedules.enabled` 与全局 `[scheduler].enabled`（决定 in-process runner 是否存在）是两个不同开关。 |
