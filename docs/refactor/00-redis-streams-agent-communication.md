...
`stop` / `cleanup_logs` 按 `attempt_id` 幂等：agent 多次收到（如 reconcile 路径与 event_stall 路径对同一 attempt 各投一次 stop）时，对已停止/不存在的进程或已清理的文件幂等忽略，不视为故障。控制命令与 `run` 一样经 command stream 投递。
...
- **进程仍在** → 重发 `attempt.running`；server 收到后先投递 `stop`，待真实 terminal 事件或 drain timeout 后再下发 `cleanup_logs`。
...
- 取消处于 `dispatch_unknown`（或任意未 sent 但可能已投递）的 attempt 时，server 投 `stop`，待 agent 回 terminal/确认无进程后才终结 `canceled`，不留"execution canceled 而 agent 仍在跑"的僵尸。
